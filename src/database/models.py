...
class TrackedOrder(Base):
    """Tracks order lifecycle: submitted -> filled/rejected/cancelled (1.4)."""
    ...
    submitted_price: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    ...
    def __repr__(self) -> str:
        return (
            f"<TrackedOrder(id={self.order_id}, symbol={self.symbol}, "
...
def get_session():
    """Get a database session (legacy, prefer get_db_session context manager)."""
