...
from src.backtest.metrics import calculate_metrics as calc_backtest_metrics
from src.backtest.metrics import calculate_daily_metrics
...
def _deduplicate_sell_trades(trades_df: pd.DataFrame) -> pd.DataFrame:
    """Deduplicate partial fills by grouping on order_id (Fix #15).

    When an order fills in multiple parts, each partial fill is recorded
    as a separate Trade row.  Summing realized_pnl across all rows would
    double-count.  We aggregate per order_id so each logical trade appears
    once with the correct total P&L.
    """
    sell_trades = trades_df[trades_df["side"] == "sell"].copy()
    if sell_trades.empty:
        return sell_trades

    # If order_id is missing/null, fall back to treating each row as unique
    if "order_id" not in sell_trades.columns or sell_trades["order_id"].isna().all():
        return sell_trades

    # Group by order_id; keep first row's metadata, sum quantities and pnl
    grouped = sell_trades.groupby("order_id", dropna=False).agg({
        "id": "first",
        "symbol": "first",
        "side": "first",
        "quantity": "sum",
        "price": "last",          # use final fill price
        "total": "sum",
        "pnl": "sum",
        "pnl_pct": "last",        # use final row's pct (most accurate)
        "strategy": "first",
        "reason": "first",
        "paper": "first",
        "executed_at": "last",     # timestamp of last fill
    }).reset_index()
...
def calculate_metrics(trades_df: pd.DataFrame, positions_df: pd.DataFrame) -> dict:
    ...
    if not trades_df.empty:
        sell_trades = _deduplicate_sell_trades(trades_df)
...
def render_analytics(trades_df: pd.DataFrame, daily_df: pd.DataFrame):
    ...
    # Calculate metrics from trades (deduplicated for partial fills)
    sell_trades = _deduplicate_sell_trades(trades_df) if not trades_df.empty else pd.DataFrame()
...
def render_attribution(trades_df: pd.DataFrame, daily_df: pd.DataFrame):
    """Render performance attribution dashboard (3.5)."""
    st.subheader("🔬 Performance Attribution")

    if trades_df.empty:
        st.info("Not enough trade data for attribution analysis.")
        return

    sell_trades = _deduplicate_sell_trades(trades_df)
    if sell_trades.empty:
        st.info("No completed trades for attribution analysis.")
        return

    # Per-strategy P&L breakdown
    st.markdown("### Per-Strategy P&L")
    if "strategy" in sell_trades.columns:
        strategy_stats = sell_trades.groupby("strategy").agg({
            "pnl": ["sum", "count", "mean"],
        }).reset_index()
        strategy_stats.columns = ["Strategy", "Total P&L", "Trades", "Avg P&L"]

        # Win rate per strategy
        def calc_strategy_win_rate(group):
            wins = (group["pnl"] > 0).sum()
            return (wins / len(group) * 100) if len(group) > 0 else 0

        win_rates = sell_trades.groupby("strategy").apply(calc_strategy_win_rate).reset_index()
        win_rates.columns = ["Strategy", "Win Rate"]
        strategy_stats = strategy_stats.merge(win_rates, on="Strategy")

        # Chart
        fig = go.Figure()
        colors = ["#00C853" if x >= 0 else "#FF5252" for x in strategy_stats["Total P&L"]]
        fig.add_trace(go.Bar(
            x=strategy_stats["Strategy"],
            y=strategy_stats["Total P&L"],
            marker_color=colors,
            text=[f"${x:,.0f}" for x in strategy_stats["Total P&L"]],
            textposition="outside",
        ))
        fig.update_layout(title="P&L by Strategy", yaxis_title="P&L ($)")
        st.plotly_chart(fig, use_container_width=True)

        # Table
        display = strategy_stats.copy()
        display["Total P&L"] = display["Total P&L"].apply(lambda x: f"${x:,.2f}")
        display["Avg P&L"] = display["Avg P&L"].apply(lambda x: f"${x:,.2f}")
        display["Win Rate"] = display["Win Rate"].apply(lambda x: f"{x:.1f}%")
        st.dataframe(display, use_container_width=True, hide_index=True)

    # Win/Loss by day of week
    st.markdown("### P&L by Day of Week")
    if "executed_at" in sell_trades.columns:
        sell_trades["day_of_week"] = pd.to_datetime(sell_trades["executed_at"]).dt.day_name()
        day_order = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]
        day_stats = sell_trades.groupby("day_of_week")["pnl"].agg(["sum", "count", "mean"]).reset_index()
        day_stats.columns = ["Day", "Total P&L", "Trades", "Avg P&L"]
        day_stats["Day"] = pd.Categorical(day_stats["Day"], categories=day_order, ordered=True)
        day_stats = day_stats.sort_values("Day")

        fig = go.Figure()
        colors = ["#00C853" if x >= 0 else "#FF5252" for x in day_stats["Total P&L"]]
        fig.add_trace(go.Bar(
            x=day_stats["Day"],
            y=day_stats["Total P&L"],
            marker_color=colors,
        ))
        fig.update_layout(title="P&L by Day of Week", yaxis_title="P&L ($)")
        st.plotly_chart(fig, use_container_width=True)

    # Win/Loss by time of day
    st.markdown("### P&L by Hour of Day")
    if "executed_at" in sell_trades.columns:
        sell_trades["hour"] = pd.to_datetime(sell_trades["executed_at"]).dt.hour
        hour_stats = sell_trades.groupby("hour")["pnl"].agg(["sum", "count"]).reset_index()
        hour_stats.columns = ["Hour", "Total P&L", "Trades"]

        fig = go.Figure()
        colors = ["#00C853" if x >= 0 else "#FF5252" for x in hour_stats["Total P&L"]]
        fig.add_trace(go.Bar(
            x=hour_stats["Hour"],
            y=hour_stats["Total P&L"],
            marker_color=colors,
        ))
        fig.update_layout(title="P&L by Hour", xaxis_title="Hour (UTC)", yaxis_title="P&L ($)")
        st.plotly_chart(fig, use_container_width=True)
    ...
    if not daily_df.empty and "ending_balance" in daily_df.columns:
        ...
        fig.update_layout(
            ...
            yaxis=dict(range=[min(drawdown_pct.min() * 1.1, -1), 1]),
        )
        st.plotly_chart(fig, use_container_width=True)
        ...
        with col3:
            in_drawdown = (drawdown_pct < -0.1).sum()
            st.metric("Days in Drawdown", str(in_drawdown))
    ...
    if not daily_df.empty and "ending_balance" in daily_df.columns:
        ...
        if len(daily_returns) >= 30:
            ...
            fig.add_hline(y=2, line_dash="dash", line_color="darkgreen", annotation_text="Excellent")
            fig.update_layout(title="30-Day Rolling Sharpe Ratio", yaxis_title="Sharpe Ratio")
            st.plotly_chart(fig, use_container_width=True)
...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
        ["📊 Positions", "📜 Trades", "📈 Performance", "🎯 Signals", "📉 Analytics", "🔬 Attribution"]
    )
    ...
    with tab6:
        render_attribution(trades_df, daily_df)
