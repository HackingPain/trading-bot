...
from .correlation import CorrelationAnalyzer, CorrelationCheckResult
...
class RiskConfig:
    ...
    correlation_threshold: float = 0.70  # Block if correlation above this
    max_portfolio_correlation: float = 0.80  # Max avg portfolio correlation
    ...
    def from_settings(cls, settings: dict) -> "RiskConfig":
        ...
        return cls(
            ...
            correlation_threshold=risk_settings.get("correlation_threshold", 0.70),
            max_portfolio_correlation=risk_settings.get("max_portfolio_correlation", 0.80),
        )
...
class RiskManager:
    ...
    def check_all(
        ...
        # 7. Correlation check (2.4)
        results.append(self.check_correlation(positions, proposed_symbol))
    ...
    def check_sector_exposure(
        ...
        """
        Check sector concentration risk (2.5).

        Uses GICS sector mapping to enforce real sector limits.
        Blocks if adding the proposed symbol would push any sector
        above max_sector_exposure_pct.
        """
        ...
        # Get sector for proposed symbol
        proposed_sector = get_sector(proposed_symbol)
        if not proposed_sector:
            # Can't determine sector, allow the trade
            return RiskCheckResult(
                check_type=RiskCheck.SECTOR_EXPOSURE,
                passed=True,
                message=f"Sector unknown for {proposed_symbol}, allowing trade",
            )
        ...
        if total_value <= 0:
            return RiskCheckResult(
                check_type=RiskCheck.SECTOR_EXPOSURE,
                passed=True,
                message="No existing position value, sector check passed",
            )
        ...
        # Check what the proposed sector exposure would be
        # (approximate: use average position size as proxy for new position)
        current_sector_value = sector_values.get(proposed_sector, 0)
        current_sector_pct = current_sector_value / total_value if total_value > 0 else 0

        # Count positions in same sector
        same_sector_count = sum(
            1 for p in positions
            if get_sector(p.symbol) == proposed_sector
        )

        if current_sector_pct >= self.config.max_sector_exposure_pct:
            return RiskCheckResult(
                check_type=RiskCheck.SECTOR_EXPOSURE,
                passed=False,
                message=(
                    f"Sector '{proposed_sector}' exposure {current_sector_pct*100:.1f}% "
                    f"exceeds limit {self.config.max_sector_exposure_pct*100:.1f}%"
                ),
                details={
                    "sector": proposed_sector,
                    "current_exposure_pct": current_sector_pct,
                    "max_exposure_pct": self.config.max_sector_exposure_pct,
                    "positions_in_sector": same_sector_count,
                },
            )
        ...
        return RiskCheckResult(
            ...
            message=(
                f"Sector '{proposed_sector}' exposure {current_sector_pct*100:.1f}% "
                f"within limit ({same_sector_count} positions)"
            ),
            details={
                "sector": proposed_sector,
                "current_exposure_pct": current_sector_pct,
    ...
    def check_correlation(
        ...
        """
        Check correlation risk for proposed trade (2.4).

        Blocks entry if the proposed symbol is highly correlated
        with too many existing positions.
        """
        held_symbols = [p.symbol for p in positions]

        if not held_symbols:
            return RiskCheckResult(
                check_type=RiskCheck.CORRELATION,
                passed=True,
                message="No existing positions to check correlation",
            )

        corr_result = self.correlation_analyzer.check_correlation(
            proposed_symbol, held_symbols
        )
        ...
        if not corr_result.passed:
            return RiskCheckResult(
                ...
                details={
                    ...
                    "high_correlations": [
                        {"symbol": c.symbol2, "correlation": c.correlation}
                        for c in corr_result.high_correlations
                    ],
        ...
        return RiskCheckResult(
            ...
            passed=True,
            message=corr_result.message,
            details={"average_correlation": corr_result.average_correlation},
        )

    def update_correlation_data(self, price_data: dict) -> None:
        """Update correlation analyzer with latest price data.

        Args:
            price_data: Dict mapping symbol to pandas Series of closing prices
        """
        self.correlation_analyzer.update_prices_batch(price_data)
